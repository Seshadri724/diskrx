...
def test_correlation_logic(mocker):
    ...
    instance.find_culprits.return_value = [ProcessRef(pid=123, name="culprit_proc", cmdline=[])]
...
def test_correlation_high_confidence(mocker):
    ...
    instance.find_culprits.return_value = [
        ProcessRef(pid=123, name="culprit_proc", cmdline=[], mode="write", files=["/growing/path/file.log"])
    ]
